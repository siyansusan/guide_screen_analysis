...
class AllPairsAnalysis(AhabAllPairs):
    ...
    def process1HSP(self, anno):
        ...
        sites = [self.ESTIMATE_FIRST_CUT_SITE, self.ESTIMATE_SECOND_CUT_SITE]

        if interval.contains(self.ESTIMATE_FIRST_CUT_SITE) or interval.contains(self.ESTIMATE_SECOND_CUT_SITE):
            if hsp.containsOnTargetIndels(sites):
    ...
    def process2HSPsSameStrand(self, anno):
        ...
        if not self.readGapSmallerThan(anno, self.MAX_READ_GAP) or anno.alignedProportion() < self.MIN_ALIGNED_PROPORTION:
            self.bin(anno, self.BIN_FAILED_FILTER)
            self.dump(anno, self.DEBUG_FAILED_FILTER)
            return
